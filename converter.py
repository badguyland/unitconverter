...
base_converter_layout = [
    [sg.Text('Enter a number:'), sg.Input(key='number')],
    [sg.Text('From base:'), sg.Input(key='from_base')],
    [sg.Text('To base:'), sg.Input(key='to_base')],
    [sg.Button('Convert')],
    [sg.Text(size=(40, 1), key='output')]
]

tab_layout = [
    [sg.TabGroup([[sg.Tab('Unit Converter', unit_converter_layout), sg.Tab('Base Converter', base_converter_layout)]])]
]

window = sg.Window('Converter', tab_layout)
...
while True:
    ...
    if event == 'Convert':
        try:
            if 'number' in values:
                num = int(values['number'], int(values['from_base']))
                result = convert_base(num, int(values['to_base']))
